...
os.system("title Zealous Selfbot - Please remember to star it - it took hours and I've released the source!")
...
spinner = Halo(
    text='ScalicOnTop#0009 | Đang Kết Nối...',
    spinner={
        'interval': 250,
        'frames': ['.', '..', '...']
    }
...
if sooo.status_code == 200:
    spinner.succeed('Token Hợp Lí | Đang Setup...')    
    ...
    print('Token Sai | Hãy thử đặt một token khác vào settings.txt') 
    try:
...
spinner = Halo(
    text=' | Zealous Selfbot - Connecting',
    spinner={
        'interval': 250,
        'frames': ['.', '..', '...']
    }
...
def screen():
    ...
    while True:
        ...
        os.system(f"title Zealous Selfbot - Running on {scalic.user.name} - Enjoy")
        logo = f"""
...
async def colorinfo(ctx,hexcolor=None):
    ...
    embed.set_footer(text="Zealous#2005")
...
async def suggest(ctx,*,message="Supply something to suggest lol!"):
    ...
    try:
        embed=discord.Embed(title=f"Zealous selfbot - poll", description=f"Poll message : `{message}`", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
...
async def ip(ctx, *, iparg="1.1.1.1"):
    ...
    personalip = requests.get("https://api64.ipify.org/?format=text").text 
    ...
    if personalip in desc:
        ...
        embed=discord.Embed(title=f"Zealous selfbot - ip locator", description="Your ip was detected in this data - to double check that your ok with showing the data\npress the reaction button below!", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        ...
        while reactionstuffyes:
            try:
                ...
                embed=discord.Embed(title=f"Zealous selfbot - ip locator", description=desc, color=randcolor)
                embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                ...
                embed=discord.Embed(title=f"Zealous selfbot - Ip locator", description="You took too long to react - try running this command again or sum", color=randcolor)
                embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                embed.set_footer(text="LOL")
                await message.edit(embed=embed)
...
async def tokeninfo(ctx, bokenxd):
    ...
    if data.status_code == 200: 
        ...
        embed=discord.Embed(title=f"Zealous selfbot - Token info", description=f"Site responded with status code : `{data.status_code}`\nMessage : `{data.text}`", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.send(embed=embed)
...
async def ping(ctx,afkstatus=None):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title=f"Zealous selfbot - ping pong ping pong ping pong ping pong ping pong ping pong ping pong ping pong ping pong ping pong", description=f'🏓 pong | {round(scalic.latency * 1000)}ms', color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.send(embed=embed)
...
Categories : `System` , `Utility` , `Emoji` , `Mod` , `Snipe` , `Nuke` , `Fun` , `Text`.
...
"""   
    ...
    embed=discord.Embed(title=f"Zealous Selfbot", description=description, color=0x36393f)
    await ctx.send(embed=embed)
...
async def afk(ctx,afkstatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Afk auto messager", description=f"Afk messages are now : `{isafk}`\nTry {prefix.strip()}afkmessage [message]", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="LOL")
    await ctx.message.edit(content="",embed=embed)

@scalic.command(aliases=['afkmessage', 'afkmessageedit'])
async def afkmsg(ctx,*,afkstatus="Bố mày đang afk"):
    global messagetosendwhenafk 

    messagetosendwhenafk = afkstatus

    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Afk auto messager", description=f"Afk messages are : `{isafk}`\nAfk message is : `{messagetosendwhenafk}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def uptime(ctx):
    ...
    seconds = timedata - 60*minutes
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Uptime", description=f"Scalic has been running for `{'%d:%d:%d' %(hours,minutes,seconds)}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def gamecycle(ctx):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Game Cycle", description=f"Cycling these statuses:\n`{statusdata}`\nDelay:\n`{delaybetweencycle}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
    while True:
...
async def biocycle(ctx):
    ...
    global delaybetweencycle
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Bio Cycle", description=f"Cycling these statuses:\n`{statusdata}`\nDelay:\n`{delaybetweencycle}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def watchcycle(ctx):
    ...
    global delaybetweencycle
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Watch Cycle", description=f"Cycling these statuses:\n`{statusdata}`\nDelay:\n`{delaybetweencycle}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="lol")
    await ctx.message.edit(content="",embed=embed)
    while True:
...
async def nicknamecycle(ctx,statustocycle=None):
    ...
    if statustocycle == None or statustocycle.lower() == "yes" or statustocycle.lower() == "on":
        ...
        embed=discord.Embed(title="Zealous Selfbot - Nickname App1e", description=f"Cycle status : `{nicknamestuffyes}`\n{prefix.strip()}nicknamecycle **to start the cycle**", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.message.edit(content="",embed=embed)
...
async def nicknamecycledelay(ctx,newdelay="5"):
    ...
    embed=discord.Embed(title="Zealous Selfbot - Nickname Delay", description=f"Delay is now : `{delaybetweennicknamecycle} seconds`\n{prefix.strip()}nicknamecycle", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def cycledelay(ctx,newdelay="5"):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Cycle Delay", description=f"Delay is now : `{delaybetweencycle} seconds", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def listencycle(ctx):
    ...
    global delaybetweencycle
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Listen Cycle", description=f"Cycling these statuses:\n`{statusdata}`\nDelay:\n`{delaybetweencycle}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
    while True:
...
async def streamcycle(ctx):
    ...
    global streamurl
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Stream Cycle", description=f"Cycling these statuses:\n`{statusdata}`\nDelay:\n`{delaybetweencycle}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
    while True:
...
async def listen(ctx, *, listenstatus="I didn't know how to specify a listen status lol"):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    await scalic.change_presence(activity=discord.Activity(type=discord.ActivityType.listening, name=listenstatus))
    embed=discord.Embed(title="Zealous Selfbot - Listen Status", description=f"Status is now : `Listening to {listenstatus}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def game(ctx, *, gamestatus="I didn't know how to specify a game status lol"):
    ...
    embed=discord.Embed(title="Zealous Selfbot - Game Status", description=f"Status is now : `Playing {gamestatus}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def watch(ctx, *, watchstatus ="I didn't know how to specify a watch status lol"):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    await scalic.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=watchstatus))    
    embed=discord.Embed(title="Zealous Selfbot - Watch Status", description=f"Status is now : `Watching {watchstatus}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def stream(ctx, *, streamstatus): 
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    await scalic.change_presence(activity=stream)  
    embed=discord.Embed(title="Zealous Selfbot - Stream Status", description=f"Status is now : `Streaming {streamstatus}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def twitchurl(ctx, streamurltouse): 
    ...
    embed=discord.Embed(title="Zealous Selfbot - Stream Url Changed", description=f"Streamurl is now : `{streamurl}`\nTry {prefix.strip()}stream or {prefix.strip()}streamcycle", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def bitcoin(ctx):
    ...
    dolor = j['USD']
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Bitcoin command", description=f"Bitcoin value : `${dolor}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def invite(ctx): 
    ...
    embed=discord.Embed(title="Zealous Selfbot -Invite Creation", description=f"Invite Made - https://discord.gg/{str(inv.json()['code'])}", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def timetodeleteafter(ctx,deletetime):
    ...
    embed=discord.Embed(title="Zealous Selfbot - Delete after time Edited", description=f"Delete after time is now : `{deletetime} seconds`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
def spam(webhook):
    ...
    while spammingdawebhookeroos:
        ...
        randcolor = random.randint(0x000000, 0xFFFFFF)
        data = {
          "content": "@everyone Mấy con gà chơi freefire",
          "embeds": [
            {
              "title": "Server này đã bị pay mau`C:",
              "tts": "true",
              "description": "Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác ",
              "url": "https://www.youtube.com/watch?v=X_C26M6MJiY",
              "color": randcolor,
              "fields": [
                {
                  "name": "Nudeeeee",
                  "value": "Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác Rác "
                },
                {
                  "name": "Rác Rác Rác ",
...
async def hypesquad(ctx,squad=None):
    ...
    if squad == None:
        ...
        if data.status_code == 204:
            embed=discord.Embed(title="Zealous Selfbot - Hypesquad Changer", description=f"Hypesquad changed successfully", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="Zealous#2005")
            await ctx.message.edit(content="",embed=embed)
        else:
            embed=discord.Embed(title="Zealous Selfbot - Hypesquad Changer", description=f"Error - Site responded with status code : `{data.status_code}`\nMessage : `{data.text}`", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="Zealous#2005")
            await ctx.message.edit(content="",embed=embed)
...
async def fucktoken(ctx,tokentofrick=None,*,nukemsg=f"This account was nuked with the help of scalic! - GG"):
    ...
    if tokentofrick == None:
        embed=discord.Embed(title="Zealous Selfbot - Token Fuck", description=f"Supply a token - {prefix.strip()}tokenfuck [token-here] [message-to-nuke-with]", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.message.edit(content="",embed=embed)
        ...
        embed=discord.Embed(title="Zealous Selfbot - Token Fuck", description=f"Try do this command in a private server - it has problems in dms!", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.message.edit(content="",embed=embed)

    else:
        embed=discord.Embed(title="Zealous Selfbot - Token Fuck", description=f"If your sure you want to token fuck the account, react with the emoji below.\nThis process will dm all open dms , then close them, leave all servers, and make a ton of servers", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        message = await ctx.send(embed=embed)        
        await message.add_reaction('✅')
        ...
        while reactionstuffyes:
            try:
                reaction, user = await scalic.wait_for('reaction_remove', timeout=10, check=requirements)
                embed=discord.Embed(title="Zealous Selfbot - Token Fuck", description=f"You reacted, the process has started!\nValidating token...", color=randcolor)
                embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                embed.set_footer(text="Zealous#2005")
                await message.edit(embed=embed)
                await message.clear_reactions()
                ...
                if tokendata.status_code != 200:
                    embed=discord.Embed(title="Zealous Selfbot - Token Fuck", description=f"Error - are you sure that token is correct?\nDiscord responded with : `{tokendata.text}`", color=randcolor)
                    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                    embed.set_footer(text="Zealous#2005")
                    await message.edit(embed=embed)           
                else:
                    embed=discord.Embed(title="Zealous Selfbot - Token Fuck", description=f"Token valid - continuing the process", color=randcolor)
                    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                    embed.set_footer(text="Zealous#2005")
                    await message.edit(embed=embed)      
                    ...
                    randcolor = random.randint(0x000000, 0xFFFFFF)
                    embed=discord.Embed(title="Zealous Selfbot - Token Fuck", description=f"Messaged {usersmessaged} people saying : `{nukemsg}` and cleared the conversation after", color=randcolor)
                    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                    embed.set_footer(text="Zealous#2005")
                    await message.edit(embed=embed)              
                    ...
                    randcolor = random.randint(0x000000, 0xFFFFFF)
                    embed=discord.Embed(title="Zealous Selfbot - Token Fuck", description=f"Left `{serversleft}` servers", color=randcolor)
                    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                    embed.set_footer(text="Zealous#2005")
                    await message.edit(embed=embed)  
                    ...
                    randcolor = random.randint(0x000000, 0xFFFFFF)
                    embed=discord.Embed(title="Zealous Selfbot - Token Fuck", description=f"Deleted `{serversdeleted}` servers", color=randcolor)
                    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                    embed.set_footer(text="Zealous#2005")
                    try:
                        await message.edit(embed=embed)  
                    ...
                    randcolor = random.randint(0x000000, 0xFFFFFF)
                    embed=discord.Embed(title="Zealous Selfbot - Token Fuck", description=f"Made {serversmade} servers", color=randcolor)
                    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                    embed.set_footer(text="Zealous#2005")
                    try:
                        await message.edit(embed=embed)  
                ...
                embed=discord.Embed(title="Zealous Selfbot - Token Fuck", description=f"You took too long - Run this command again if you wish to token fuck an account", color=randcolor)
                embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                embed.set_footer(text="Zealous#2005")
                await message.edit(embed=embed)
                await message.clear_reactions()
...
async def tokendisable(ctx,tokentofrick=None):
    ...
    await ctx.message.delete()
    if tokentofrick == None:
        embed=discord.Embed(title="Zealous Selfbot - Token Disabler", description=f"Supply a token - {prefix.strip()}tokendisable [token-here] ", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.message.edit(content="",embed=embed)
        ...
        embed=discord.Embed(title="Zealous Selfbot - Token Disabler", description=f"Try do this command in a private server - it has problems in dms (working on fixing)!", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.message.edit(content="",embed=embed)

    else:
        embed=discord.Embed(title="Zealous Selfbot - Token Disabler", description=f"If your sure you want to disable that token, react.\nDisabling means that the account will be DISABLED - the chances of recovering it are pratically impossible.", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        message = await ctx.send(embed=embed)        
        await message.add_reaction('✅')
        ...
        while reactionstuffyes:
            try:
                reaction, user = await scalic.wait_for('reaction_remove', timeout=10, check=requirements)
                embed=discord.Embed(title="Zealous Selfbot - Token Disabler", description=f"You reacted, the process has started!\nDisabling token", color=randcolor)
                embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                embed.set_footer(text="Zealous#2005")
                await message.edit(embed=embed)
                await message.clear_reactions()
                ...
                randcolor = random.randint(0x000000, 0xFFFFFF)
                if tokendata.status_code != 200:
                    embed=discord.Embed(title="Zealous Selfbot - Token Disabler", description=f"Token disabled - discord responds with : `{tokendata.text}`", color=randcolor)
                    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                    embed.set_footer(text="Zealous#2005")
                    await message.edit(embed=embed)           
                else:
                    embed=discord.Embed(title="Zealous Selfbot - Token Disabler", description=f"Token valid - strange - try again maybe?", color=randcolor)
                    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                    embed.set_footer(text="Zealous#2005")
                    await message.edit(embed=embed)      

            except asyncio.TimeoutError:
                embed=discord.Embed(title="Zealous Selfbot - Token Disabler", description=f"You took too long - Run this command again if you wish to DISABLE an account", color=randcolor)
                embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                embed.set_footer(text="Zealous#2005")
                await message.edit(embed=embed)
                await message.clear_reactions()
...
async def editsnipe(ctx):
    ...
    if str(ctx.channel.id) in editedmessages:
        ...
        embed=discord.Embed(title="Zealous Selfbot - Edit snipe", description=f"Message sniped:\n{msgsniped}", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.message.edit(content="",embed=embed)

    else:
        embed=discord.Embed(title="Zealous Selfbot - Edit snipe", description=f"No edited messages to snipe", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.message.edit(content="",embed=embed)
...
async def unban(ctx, userid="Nonexd"):
    ...
    if len(str(userid)) != 18:
        embed=discord.Embed(title="Zealous Selfbot - Unban Command", description=f"\nCommand usage : {prefix.strip()}unban [id]", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.message.edit(content="",embed=embed)   
    else:
...
async def banid(ctx, userid="Nonexd",reason="None specified"):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    if len(str(userid)) != 18:
        embed=discord.Embed(title="Zealous Selfbot - Ban ID Command", description=f"\nCommand usage : {prefix.strip()}banid [id]", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.message.edit(content="",embed=embed)   
    else:
        ...
        try:
            ...
            embed=discord.Embed(title="Zealous Selfbot - Ban Command", description=f"\nBanned {userid}", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="Zealous#2005")
            await ctx.message.edit(content="",embed=embed)   
...
async def kick(ctx, member : discord.Member,reason="None specified"):
    ...
    if member.id == ctx.author.id:
        embed=discord.Embed(title="Zealous Selfbot - Kick Command", description=f"\nTry {prefix.strip()}leave [server-id] instead!", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.message.edit(content="",embed=embed) 
    else:
...
async def downloadguildemojis(ctx, guildid=None):
    if guildid == None:
        ...
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title=f"Zealous Selfbot - Emoji stealing from {guildtostealfrom.name}", description=f"Details:\nSuccessful emoji steals : {emojisuccess}\nErrors with emoji steals : {emojierror}", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.message.edit(content="",embed=embed)
        ...
        for emoji in guildtostealfrom.emojis:
            ...
            try:
                response = requests.get(emoji.url, stream=True)
                with open(f"./data/{emoji.name}.jpeg", 'wb') as scalic_file: 
                    shutil.copyfileobj(response.raw, scalic_file)
...
async def stealguildemoji(ctx, guildid=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title=f"Zealous Selfbot - Finished emoji stealing from {guildtostealfrom.name}", description=f"Details:\nSuccessful emoji steals : {emojisuccess}\nErrors with emoji steals : {emojierror}\n{emojilist}", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(embed=embed)
...
async def embedspam(ctx, count=5, *, desc="I forgot to specify the embed content"):
    ...
    if int(count) > 25:
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Zealous Selfbot - embed spam", description=f"{count} embeds is too much - do 25 or less", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.send(embed=embed)
    else:
        for i in range(int(count)):
            randcolor = random.randint(0x000000, 0xFFFFFF)
            embed=discord.Embed(title="Zealous Selfbot - embed spam", description=desc, color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="Zealous#2005")
            await ctx.send(embed=embed)
...
async def spam(ctx, count=5, *, desc="I forgot to specify the content to spam"):
    ...
    if int(count) > 25:
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Zealous Selfbot - spam", description=f"{count} messages is too much - do 25 or less", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.send(embed=embed)
    else:
...
async def spampin(ctx, count=None):
    if count == None:
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Zealous Selfbot - Pin spam", description=f"You didn't specify the amount of pins to do.\n{prefix.strip()}spampin [amount]", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
...
async def logout(ctx):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Logging out", description=f"Logging out!", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
    await scalic.logout()
...
async def clear(ctx, amount):
    ...
    await ctx.message.delete()
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Message purger", description=f"Deleting messages", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Message purger", description=f"Finished.\nDeleted {deleted} messages", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
...
async def spamreact(ctx, count=None, reaction=None):
    ...
    if count == None or reaction == None:
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Zealous Selfbot - React spam", description=f"You didn't specify the amount of messages to react to or the reaction to use.\n{prefix.strip()}spamreact [amount]", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
...
async def spamedit(ctx, count=None,*, mesg=None):
    ...
    if count == None or mesg == None:
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Zealous Selfbot - Message edit spam", description=f"You didn't specify the amount of messages to edit or the content to edit the messages to.\n{prefix.strip()}spamedit [amount] [message]", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        ...
        edited = 0
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Zealous Selfbot - Message edit spam", description=f"Editing messages", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        msg= await ctx.send(embed=embed)
        ...
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Zealous Selfbot - Message edit spam", description=f"Finished.\nEdited {edited} messages", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await msg.edit(embed=embed,delete_after=3)
...
async def tokenhalf(ctx, member: discord.Member):#
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Message edit spam", description=f"{member.name}'s Token Begins With : \n`{final[1:]}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def logdeletedmessages(ctx,deletestatus=None):
    ...
    if deletestatus == None:
        ...
        if deletestatus.lower() == "true": 
            deletedmessagelogger = "on"
        if deletestatus.lower() == "false":
            deletedmessagelogger = "off"

    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Deleted message logger", description=f"Deleted message logger is now : `{deletedmessagelogger}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def logediteddmessages(ctx,editstatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Edited message logger", description=f"Edited message logger is now : `{editedmessagelogger}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def edit(ctx,editstatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Edit mode", description=f"Edit mode is now : `{statusofediting}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)#
...
async def talkcute(ctx,talkcuteyems=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Cute talk", description=f"Edit mode is now : `{talkingcuteyems}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def nitrosnipe(ctx,snipestatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Nitro Sniper", description=f"Nitro sniper is now : `{nitrosniping}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def privnotesnipe(ctx,snipestatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Privnote Sniper", description=f"Privnote sniper is now : `{privnotesniping}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def notifications(ctx,notifstatushm=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Notifications", description=f"Notifications are now : `{notifsyems}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def giveawaysnipe(ctx,snipestatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Giveaway Sniper", description=f"Giveaway sniper is now : `{giveawaysniping}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def backup(ctx,snipestatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Backup", description=f"Backing up friends...", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Backup", description=f"Saved `{friendssaved}` friends", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Backup", description=f"Saved `{friendssaved}` friends and `{serverssaved}` servers", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
    ...
    for cha in scalic.private_channels:
        if isinstance(cha, discord.GroupChannel):
            inv = requests.post(f'https://discord.com/api/v6/channels/{cha.id}/invites', json = {"max_age":86400}, headers = headers) 
            try:
                invitecode = f"discord.gg/{str(inv.json()['code'])}"
            except:
    ...
    f.close()
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Backup", description=f"Saved `{friendssaved}` friends , `{serverssaved}` servers and `{groupssaved}` group chats.", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def pastebinsnipe(ctx,snipestatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Pastebin Sniper", description=f"Pastebin sniper is now : `{pastebinsniping}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def idtoname(ctx, personsid: int):
    if len(str(personsid)) != 18:
        ...
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Zealous Selfbot - Id to username", description=f"ID [{str(personsid)}]  = `{user.name}#{user.discriminator}`", color=randcolor)
...
async def hastebin(ctx, *,paste=f"Format is {prefix.strip()}hastebin [text]"):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Hastebin Command", description=f"Here's your paste!\n{textlink}{endoflink}", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/dKfTyqLt1jkqIfiMXj/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def nuke(ctx):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Channel Nuked", description=f"<#{new_chan.id}> - {new_chan.name} has been nuked", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/dKfTyqLt1jkqIfiMXj/giphy.gif")
    embed.set_footer(text="Zealous#2005")
...
async def on_message(ctx):
    ...
    if mimicstatus == "on":
        if ctx.author.id == whotomimic:
            try:
                await ctx.channel.send(ctx.content.replace("@","@\u200b")) 
            except Exception as e:
                pass
...
async def on_message_edit(before,after):
    global editedmessages
    global editedmessagelogger
    editedmessages.update({after.channel.id: before.content.replace("@","@\u200b")}) 
...
async def tinyurl(ctx,link=None):
    ...
    await ctx.message.delete()
    if link == None:
        embed=discord.Embed(title=f"Zealous Selfbot - Url shortener", description=f"You didn't supply a link - {prefix.strip()}tinyurl [link-here]", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        message = await ctx.send(embed=embed)
    else:
        embed=discord.Embed(title=f"Zealous Selfbot - Shortening link", description=f"beep bop beep", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        message = await ctx.send(embed=embed)
        ...
        embed=discord.Embed(title=f"Zealous Selfbot - Url shortener", description=f"Link shortened : \n{shortened}", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        message = await ctx.send(embed=embed)
...
async def join(ctx,link=None):#
    await ctx.message.delete()
    if link == None:
        embed=discord.Embed(title=f"Zealous Selfbot - Server joiner", description=f"You didn't supply an invite - {prefix.strip()}join discord.gg/server", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.send(embed=embed)
    else:
        ...
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title=f"Zealous Selfbot - Server joiner", description=f"Joining server : `discord.gg/{link}`", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        message = await ctx.send(embed=embed)
        ...
        if joint == 200:
            ...
            embed=discord.Embed(title=f"Zealous Selfbot - Server joiner", description=f"Successfully joined : `discord.gg/{link}`", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="Zealous#2005")
            

        else:
            embed=discord.Embed(title=f"Zealous Selfbot - Server joiner", description=f"Error Joining : `discord.gg/{link}`", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="Zealous#2005")
        await message.edit(embed=embed)
...
async def leave(ctx,servid=None):#
    ...
    if servid == None:
        ...
        embed=discord.Embed(title=f"Zealous Selfbot - Server Leaver", description=f"Leaving `{servid}`", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        message = await ctx.send(embed=embed)
        ...
        if leave.status_code == 204:
            ...
            embed=discord.Embed(title=f"Zealous Selfbot - Server Leaver", description=f"Success | Left Server : `{servid}`", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="Zealous#2005")
            await message.edit(embed=embed)
            ...
            embed=discord.Embed(title=f"Zealous Selfbot - Server Leaver", description=f"Error | Error leaving server : `{servid}`\nMessage : {leave.text}", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="Zealous#2005")
            await message.edit(embed=embed)
...
async def serverinfo(ctx):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title=f"Zealous Selfbot - Gathering info on {ctx.guild.name}", description="...", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    message = await ctx.send(embed=embed)
    await asyncio.sleep(1)
...
"""
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title=f"Zealous Selfbot - {ctx.guild.name}", description=info, color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await message.edit(embed=embed)
...
async def userinfo(ctx, member: discord.Member = None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - User info", description=f"Getting user info on {member.name}", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    message = await ctx.send(embed=embed)
    await asyncio.sleep(2)
...
"""  
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title=f"Zealous Selfbot - {member.name}#{member.discriminator}", description=info, color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await message.edit(embed=embed)
...
async def dmclear(ctx):
    ...
    await ctx.message.delete()
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Zealous Selfbot - Message Clearer", description=f"Clearing all messages with all users", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    msg= await ctx.send(embed=embed)
    for channel in scalic.private_channels:
        ...
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Zealous Selfbot - Message Clearer", description=f"Clearing all messages with all users\nUsers Done : {usersdone}\nTotal Messages Deleted : {totalmessage}", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
...
async def joke(ctx):
    ...
    embed=discord.Embed(title="Zealous Selfbot - Joke requested", description=f"{setup}\n||{delivery}||", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="Zealous#2005")
    await ctx.message.edit(content="",embed=embed)
...
async def ud(ctx,*,wordtodefine=None):
    ...
    if wordtodefine == None:  
        embed=discord.Embed(title="Zealous Selfbot - Urban Dictionary Command", description=f"You didn't supply a word to define?", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.message.edit(content="",embed=embed)
    else:
        ...
        try:
            ...
            embed=discord.Embed(title="Zealous Selfbot - Urban Dictionary Command", description=f"Definition of `{wordtodefine}`\n{finaldef}\n\n", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="Zealous#2005")
            await ctx.message.edit(content="",embed=embed)
        except:
            embed=discord.Embed(title="Zealous Selfbot - Urban Dictionary Command", description=f"Error when searching for the term : `{wordtodefine}`", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="Zealous#2005")
            await ctx.message.edit(content="",embed=embed)           
...
@scalic.command(aliases=['catcha',"capture"]) 
...
async def name(ctx,*,namesupplied=None):
    ...
    if namesupplied == None:
        ...
        embed=discord.Embed(title="Zealous Selfbot - Gender command", description=f"Majority of people named {namesupplied} are {gen}", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.message.edit(content="",embed=embed)
...
async def webhookdelete(ctx,link=None):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    if link == None:
        embed=discord.Embed(title="Zealous Selfbot - No webhook supplied", description=f"{prefix.strip()}webhookdelete [webhook]", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.message.edit(content="",embed=embed)
    else:
        embed=discord.Embed(title="Zealous Selfbot - Deleting webhook", description=f"Sending a delete request to\n{link}", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="Zealous#2005")
        await ctx.message.edit(content="",embed=embed)
        ...
        if result.status_code == 204:
            embed=discord.Embed(title="Zealous Selfbot - Webhook Deleted", description=f"Yay", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="Zealous#2005")
            await ctx.message.edit(embed=embed)
        else:
            embed=discord.Embed(title="Zealous Selfbot - Error Deleting Webhook", description=f"Delete request responded with status code : {result.status_code}\{result.text}", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="Zealous#2005")
            await ctx.message.edit(embed=embed)
